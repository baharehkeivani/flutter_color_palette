...
class ColorPickerLabelState extends State<ColorPickerLabel> {
  ...
  void _updateColor(String item, int itemValue) {
    int alphaIndex = _colorTypes[_colorType]!.indexOf("A");
    int hueIndex = _colorTypes[_colorType]!.indexOf("H");
    int saturationIndex = _colorTypes[_colorType]!.indexOf("S");
    int valueIndex = _colorTypes[_colorType]!.indexOf("V");
    int lightnessIndex = _colorTypes[_colorType]!.indexOf("L");
    int redIndex = _colorTypes[_colorType]!.indexOf("R");
    int greenIndex = _colorTypes[_colorType]!.indexOf("G");
    int blueIndex = _colorTypes[_colorType]!.indexOf("B");

    int h = hueIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[hueIndex])
        : 0;
    int s = saturationIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[saturationIndex])
        : 0;
    int v = valueIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[valueIndex])
        : 0;
    int l = lightnessIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[lightnessIndex])
        : 0;
    int r = redIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[redIndex])
        : 0;
    int g = greenIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[greenIndex])
        : 0;
    int b = blueIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[blueIndex])
        : 0;
    int a = alphaIndex != -1
        ? int.parse(colorValue(widget.hsvColor, _colorType)[alphaIndex])
        : 0;

    switch (item) {
      case "H":
        if (_validateRange(itemValue, 0, 359)) {
          h = itemValue;
        }
        break;
      case "S":
        if (_validateRange(itemValue, 0, 100)) {
          s = itemValue;
        }
        break;
      case "V":
      ...
      case "L":
        ...
        break;
      case "R":
        if (_validateRange(itemValue, 0, 255)) {
          r = itemValue;
        }
        break;
      case "G":
    ...
    switch (_colorType) {
      case ColorLabelType.rgb:
        color = Color.fromARGB(a * 255 ~/ 100, r, g, b);
        break;
                  ...
                  children: <Widget>[
                    ...
                    TextField(
                      ...
                      inputFormatters: <TextInputFormatter>[
                        FilteringTextInputFormatter.allow(_endRangeRegex(item)!)
                      ],
                      style: widget.textStyle ??
  ...
  bool _validateRange(int value, int start, int end) {
    if (start <= end) {
  ...
  Pattern? _endRangeRegex(String item) {
    switch (item) {
      case "R":
      case "G":
      case "B":
      ...
      case "L":
      case "A":
        return RegExp(r'^(100|[0-9]|[1-9][0-9])$');
      default:
        return null;
    }
  }
